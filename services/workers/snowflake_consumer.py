...
class SnowflakeConsumer:
    ...
    def __init__(self):
        ...
        self.batch = []
        self.batch_size = 20
        self.dlq_producer = KafkaProducerService()
        ...
        if self.consumer is None:
            ...
            for i in range(retries):
                try:
                    ...
                    logger.warning(f"Consumer init failed {i+1}/{retries}: {e}")
                    await asyncio.sleep(2 ** i)
            raise Exception("Consumer init failed")
    ...
    async def _init_snowflake(self):
        ...
        for i in range(5):
            try:
                self.snowflake = SnowflakeAsyncService()
                logger.info("Snowflake ready")
                return
            except Exception as e:
                logger.warning(f"Snowflake init failed {i+1}/5: {e}")
                await asyncio.sleep(2 ** i)
        raise Exception("Snowflake init failed")
    ...
    async def start(self):
        ...
        try:
            ...
            logger.warning("Snowflake unavailable")
    ...
    async def _flush(self):
        try:
            if not self.snowflake:
                logger.warning("Skipping flush: Snowflake not available")
                return

            await self.snowflake.insert_batch(self.batch)
            await self.consumer.commit()

            logger.info(f"Flushed {len(self.batch)} events")
            self.batch.clear()
